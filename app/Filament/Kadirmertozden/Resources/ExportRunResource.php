    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                // YENİ: Kaydedilen gerçek storage yolu
                Tables\Columns\TextColumn::make('storage_path')
                    ->label('Dosya Yolu (storage)')
                    ->formatStateUsing(function (?string $state, ExportRun $r) {
                        // storage_path boşsa varsayılan stratejiyi göster
                        return $state ?: ('exports/' . $r->publish_token . '.xml');
                    })
                    ->copyable()
                    ->limit(60)
                    ->toggleable(),
